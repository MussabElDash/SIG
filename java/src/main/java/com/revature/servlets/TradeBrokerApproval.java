...
public class TradeBrokerApproval extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(tdao.updateTrades(t)) {
			if(approval < 0l) {
				log.info("Trade [ Trade ID: " + t.getId() + " ] successfully DENIED by broker.");
			}
			else if(approval > 0l) {
				log.info("Trade [ Trade ID: " + t.getId() + " ] successfully APPROVED by broker.");
			...
			log.error("ERROR OCCURED in setting broker approval on trade [ Trade ID: " + t.getId() + " ]");
		}
